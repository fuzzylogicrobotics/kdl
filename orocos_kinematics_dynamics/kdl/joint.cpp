...
namespace KDL {
    ...
    Joint::Joint(const std::string& _name, const JointType& _type, const double& _scale, const double& _offset,
                 const double& _inertia, const double& _damping, const double& _stiffness, const double& _upper_position_limit, const double& _lower_position_limit,const double& _home):
      name(_name),type(_type),scale(_scale),offset(_offset),inertia(_inertia),damping(_damping),stiffness(_stiffness),upper_position_limit(_upper_position_limit),lower_position_limit(_lower_position_limit),home_position(_home)
    {
    ...
    Joint::Joint(const JointType& _type, const double& _scale, const double& _offset,
                 const double& _inertia, const double& _damping, const double& _stiffness, const double& _upper_position_limit, const double& _lower_position_limit,const double& _home):
      name("NoName"),type(_type),scale(_scale),offset(_offset),inertia(_inertia),damping(_damping),stiffness(_stiffness),upper_position_limit(_upper_position_limit),lower_position_limit(_lower_position_limit),home_position(_home)
    {
      if (type == RotAxis || type == TransAxis) throw joint_type_ex;
      q_previous = 0;
    }
    ...
    Joint::Joint(const std::string& _name, const Vector& _origin, const Vector& _axis, const JointType& _type, const double& _scale, 
                 const double& _offset, const double& _inertia, const double& _damping, const double& _stiffness, const double& _upper_position_limit, const double& _lower_position_limit,const double& _home):
      name(_name), type(_type),scale(_scale),offset(_offset),inertia(_inertia),damping(_damping),stiffness(_stiffness),upper_position_limit(_upper_position_limit),lower_position_limit(_lower_position_limit),home_position(_home)
  ...
  double Joint::getUpperPositionLimit() const
  {
      return upper_position_limit;
  }

  double Joint::getLowerPositionLimit() const
  {
      return lower_position_limit;
  }
  ...
  const double &KDL::Joint::getHomePosition() const
  {
      return home_position;
  }

  const double &Joint::getScale() const
  {
      return scale;
  }
