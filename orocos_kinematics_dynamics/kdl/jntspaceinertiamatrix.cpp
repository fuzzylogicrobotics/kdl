...
namespace KDL
{
    ...
    void JntSpaceInertiaMatrix::resize(std::size_t newSize)
    {
        data.resize(newSize,newSize);
    }

    double JntSpaceInertiaMatrix::operator()(std::size_t i,std::size_t j)const
    {
        return data(i, j);
    }

    double& JntSpaceInertiaMatrix::operator()(std::size_t i,std::size_t j)
    {
        return data(i, j);
    }

    std::size_t JntSpaceInertiaMatrix::rows()const
    {
        return data.rows();
    }

    std::size_t JntSpaceInertiaMatrix::columns()const
    {
        return data.cols();
    }
