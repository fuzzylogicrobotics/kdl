...
namespace KDL
{
    ...
    Jacobian::Jacobian(std::size_t nr_of_columns):
    ...
    void Jacobian::resize(std::size_t new_nr_of_columns)
    {
        data.conservativeResize(Eigen::NoChange,new_nr_of_columns);
    }

    double Jacobian::operator()(std::size_t i,std::size_t j)const
    {
        return data(i,j);
    }

    double& Jacobian::operator()(std::size_t i,std::size_t j)
    {
        return data(i,j);
    }
    ...
    bool changeRefPoint(const Jacobian& src1, const Vector& base_AB, Jacobian& dest)
    {
        if(src1.columns()!=dest.columns())
            return false;
        for(std::size_t i=0;i<src1.columns();i++)
    ...
    Twist Jacobian::getColumn(std::size_t i) const{
        return Twist(Vector(data(0,i),data(1,i),data(2,i)),Vector(data(3,i),data(4,i),data(5,i)));
    }

    void Jacobian::setColumn(std::size_t i,const Twist& t){
