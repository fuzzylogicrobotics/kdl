...
namespace KDL
{
    ...
    public:
        ...
        Jacobian();
        explicit Jacobian(std::size_t nr_of_columns);
        Jacobian(const Jacobian& arg);

        ///Allocates memory for new size (can break realtime behavior)
        void resize(std::size_t newNrOfColumns);
        ...
        Twist getColumn(std::size_t i) const;
        void setColumn(std::size_t i,const Twist& t);
