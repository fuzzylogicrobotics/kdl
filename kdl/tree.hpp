...
#define GetTreeElementSegment(tree_element) (tree_element).segment
    ...
    {
    public:
        TreeElement(const Segment& segment_in,const SegmentMap::const_iterator& parent_in,std::size_t q_nr_in):
            segment(segment_in),
            q_nr(q_nr_in),
            parent(parent_in)
        {}
        ...
        friend bool Equal(const TreeElement& lhs, const SegmentMap& lhs_map, const TreeElement& rhs, const SegmentMap& rhs_map, double eps=epsilon)
        {
            const auto eq_it = [](const SegmentMap::const_iterator& a_it, const SegmentMap& a_map, const SegmentMap::const_iterator& b_it, const SegmentMap& b_map)
            {
                auto is_same_endness = (a_it == a_map.end()) == (b_it == b_map.end());
                if (is_same_endness && a_it == a_map.end()) { return true; }
                return a_it->first == b_it->first;
            };
            if (!(Equal(lhs.segment, rhs.segment, eps)
                && lhs.q_nr == rhs.q_nr
                // && eq_it(lhs.parent, lhs_map, rhs.parent, rhs_map)
                && lhs.children.size() == rhs.children.size()))
            {
                return false;
            }
            for (std::size_t i=0, last=lhs.children.size(); i != last; ++i)
            {
                if (!eq_it(lhs.children[i], lhs_map, rhs.children[i], rhs_map))
                {
                    return false;
                }
            }
    ...
    public:
        ...
        friend bool Equal(const Tree& a, const Tree& b, double eps);
    };

    inline bool Equal(const Tree& a, const Tree& b, double eps=epsilon)
    {
        if (std::tie(a.nrOfJoints, a.nrOfSegments, a.root_name) != std::tie(b.nrOfJoints, b.nrOfSegments, b.root_name) || a.segments.size() != b.segments.size())
        {
            return false;
        }
        auto rhs_segments_it = b.segments.begin();
        for (auto lhs_segment_it = a.segments.begin(), lhs_segment_last = a.segments.end(); lhs_segment_it != lhs_segment_last; ++lhs_segment_it, ++rhs_segments_it)
        {
            if (lhs_segment_it->first != rhs_segments_it->first || !Equal(lhs_segment_it->second, a.segments, rhs_segments_it->second, b.segments, eps))
            {
                return false;
            }
        }
