...
class MyTask : public RTT::TaskContext
{
   ...
   bool configureHook()
   {
       std::size_t size = some_property.rvalue();
	   j.resize(size)
	   ...
   }
...
\endcode
    ...
    public:
         ...
         */
        explicit JntArray(std::size_t size);
        JntArray(std::initializer_list<double> l);
        explicit JntArray(const std::vector<double>& v);

        /** Copy constructor 
         * @note Will correctly copy an empty object
         */
        JntArray(const JntArray& arg);
        JntArray(JntArray&& arg) noexcept;
        JntArray(const Eigen::VectorXd& arg);
        JntArray(Eigen::VectorXd&& arg) noexcept;
        ~JntArray();
        /** Resize the array 
        ...
        JntArray& operator = ( JntArray&& arg) noexcept;
        /**
