...
double& Wrench::operator()(std::size_t i)
{
...
double Wrench::operator()(std::size_t i) const
{
    // assert((0<=i)&&(i<6)); done by underlying routines
    if (i<3) 
        return force(i);
...
double& Twist::operator()(std::size_t i)
{
    // assert((0<=i)&&(i<6)); done by underlying routines
    if (i<3) 
        return vel(i);
...
double Twist::operator()(std::size_t i) const
{
    // assert((0<=i)&&(i<6)); done by underlying routines
    if (i<3) 
        return vel(i);
...
double& Rotation::operator()(std::size_t i,std::size_t j) {
    FRAMES_CHECKI((0<=i)&&(i<=2)&&(0<=j)&&(j<=2));
    return data[i*3+j];
}

double Rotation::operator()(std::size_t i,std::size_t j) const {
    FRAMES_CHECKI((0<=i)&&(i<=2)&&(0<=j)&&(j<=2));
    return data[i*3+j];
}
...
double Frame::operator()(std::size_t i,std::size_t j) {
...
double Frame::operator()(std::size_t i,std::size_t j) const {
    FRAMES_CHECKI((0<=i)&&(i<=3)&&(0<=j)&&(j<=3));
        if (i==3) {
        if (j==3)
            return 1;
...
IMETHOD double Vector2::operator()(std::size_t index) const {
    FRAMES_CHECKI((0<=index)&&(index<=1));
    return data[index];
}

IMETHOD double& Vector2::operator () (std::size_t index)
{
    FRAMES_CHECKI((0<=index)&&(index<=1));
    return data[index];
}
...
IMETHOD double Rotation2::operator()(std::size_t i,std::size_t j) const {
...
IMETHOD double Frame2::operator()(std::size_t i,std::size_t j) {
    FRAMES_CHECKI((0<=i)&&(i<=2)&&(0<=j)&&(j<=2));
    if (i==2) {
        if (j==2)
            return 1;
...
IMETHOD double Frame2::operator()(std::size_t i,std::size_t j) const {
    FRAMES_CHECKI((0<=i)&&(i<=2)&&(0<=j)&&(j<=2));
    if (i==2) {
        if (j==2)
            return 1;
